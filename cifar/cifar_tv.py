...
def select_inputs(imgs, labels, model, scorer, jpeg):
    B, Q, C, H, W = imgs.shape
    if not jpeg or Q == 1:
        return imgs[:,0]
    high, med, low = imgs[:,0], imgs[:,1], imgs[:,2]
    with torch.no_grad():
        losses = scorer(model(high), labels)

    use_med = (losses > 0.1) & (losses <= 0.2)
    use_low = losses <= 0.1

    inputs = high.clone()
    inputs[use_med] = med[use_med]
    inputs[use_low] = low[use_low]
    return inputs
...
def main():
    ...
    train_ds = CIFAR10(root='./torch_data', train=True, download=True)
    test_ds = CIFAR10(root='./torch_data', train=False, download=True)

    dataloader = DataLoader(train_ds.data, train_ds.targets, batch_size=128, shuffle=True, jpeg=jpeg, Q=[100, 97, 90], subsampling="4:2:2", transform=None)
    validloader = DataLoader(test_ds.data, test_ds.targets, batch_size=10000, shuffle=True, jpeg=jpeg, Q=[100, 97, 90], subsampling="4:2:2", transform=None)
    ...
    for epoch in range(1, epochs+1):
        ...
        for imgs, labels in dataloader:
            ...
            inputs = select_inputs(imgs, labels, model, criterion_no_reduction, jpeg)
